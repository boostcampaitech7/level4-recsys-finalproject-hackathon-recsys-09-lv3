...
class SteambaseDataset(Dataset):
    ...
    def __init__(self, dataset_name, df, description, device):
        ...
        self.features = [name for name in df.columns if name != 'interaction']
...
class SteamColdStartDataLoader(object):
    """
    ...
    def __init__(self, dataset_name, dataset_path, device, bsz=32, shuffle=True):
        ...
        for key, df in data.items():
            if key == 'description':
                continue
            if 'metaE' not in key:
                self.dataloaders[key] = DataLoader(SteambaseDataset(dataset_name, df, self.description, device), batch_size=bsz, shuffle=shuffle)
            else:
